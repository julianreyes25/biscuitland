...
export class Collection<K, V> extends Map<K, V> {
	...
	sweeper:
		| (CollectionSweeper<K, V> & { intervalId?: number })
		| undefined;

	constructor(
		entries?: (ReadonlyArray<readonly [K, V]> | null) | Map<K, V>,
		...
		this.startSweeper(options?.sweeper as CollectionSweeper<K, V>);
	}

	startSweeper(options: CollectionSweeper<K, V>) {
	...
	set(key: K, value: V): this {
		...
		if (
			(this.maxSize !== undefined || this.maxSize === 0) &&
