...
import { delay, Logger } from '../../common/mod.ts';
...
export class DynamicBucket {
	...
	refill(): void {
		...
		this.used = this.refillAmount > this.used
			? 0
	...
	async processQueue(): Promise<void> {
		...
		while (!this.queue.isEmpty()) {
			if (this.remaining) {
				this.logger.debug(
					`Processing queue. Remaining: ${this.remaining} Length: ${this.queue.size()}`,
				);
...
export class PriorityQueue<T> extends Queue<T>
	implements QueuePusherWithPriority<T> {
...
export class SequentialBucket {
	...
	public acquire(promises = this.connections) {
