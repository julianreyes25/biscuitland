...
import { properties, ShardState } from '../constants.ts';
import { DynamicBucket, PriorityQueue } from '../structures.ts';
import { ShardHeartBeater } from './heartbeater.ts';
import { ShardData, ShardOptions, ShardSocketCloseCodes } from './shared.ts';
...
export class Shard {
	...
	connect() {
		...
		return new Promise<Shard>((resolve, reject) => {
			...
			this.websocket!.onopen = () => {
				if (
					![ShardState.Resuming, ShardState.Identifying].includes(this.state)
				) {
	...
	checkOffline(priority: number) {
		if (!this.isOpen()) {
			return new Promise((resolve) =>
				this.offlineSendQueue.push(resolve, priority)
			);
		}
	 ...
	 */
	async send<T extends GatewaySendPayload = GatewaySendPayload>(
		priority: number,
		message: T,
	) {
	...
	protected handleMessage({ data }: MessageEvent) {
	...
	protected async handleClosed(close: CloseEvent) {
		...
		switch (close.code) {
			...
			case GatewayCloseCodes.SessionTimedOut:
				this.logger.debug(
					`[Shard #${this.id}] Gateway connection closing requiring re-identify. Code: ${close.code}`,
				);
				this.state = ShardState.Identifying;
			...
			default:
				this.logger.info(
					`[Shard #${this.id}] (${close.code}) closed shard #${this.id}. Resuming...`,
				);
				this.state = ShardState.Resuming;
	...
	calculateSafeRequests(): number {
		...
		const safeRequests =
			...
			Math.ceil(
					this.options.ratelimitOptions!.rateLimitResetInterval /
						this.heartbeater!.heart.interval,
				) * 2;
