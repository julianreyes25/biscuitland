...
import { ShardSocketCloseCodes } from './shared.ts';
...
export class ShardHeartBeater {
	...
	handleHeartbeat(
		_packet: Extract<GatewayReceivePayload, GatewayHeartbeatRequest>,
	) {
		this.shard.logger.debug(
			`[Shard #${this.shard.id}] received hearbeat event`,
		);
	...
	heartbeat(acknowledgeAck: boolean) {
		if (acknowledgeAck) {
			if (!this.heart.lastAck) {
				this.shard.logger.debug(
					`[Shard #${this.shard.id}] Heartbeat not acknowledged.`,
				);
				this.shard.close(
