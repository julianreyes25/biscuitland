...
import { ShardManagerDefaults } from '../constants.ts';
import { SequentialBucket } from '../structures.ts';
import { Shard } from './shard.ts';
import { ShardManagerOptions } from './shared.ts';
...
export class ShardManager extends Collection<number, Shard> {
	...
	constructor(options: ShardManagerOptions) {
		super();
		this.options = Options<Required<ShardManagerOptions>>(
			ShardManagerDefaults,
			options,
			{ info: { shards: options.totalShards } },
		);
	...
	spawn(shardId: number) {
		...
		shard ??= new Shard(shardId, {
			...
			info: Options<APIGatewayBotInfo>(this.options.info, {
	...
	joinVoice(
		guild_id: string,
		channel_id: string,
		options: ObjectToLower<
			Pick<GatewayVoiceStateUpdate['d'], 'self_deaf' | 'self_mute'>
		>,
	) {
		const shardId = this.calculeShardId(guild_id);
		this.logger.info(
			`Shard #${shardId} join voice ${channel_id} in ${guild_id}`,
		);
