...
import { MakeRequired, Options } from '../../common/mod.ts';
import { Handler, type Session } from '../../core/mod.ts';
import { GatewayEvents } from '../../ws/mod.ts';
...
export class EventCollector<E extends keyof GatewayEvents>
	...
	start() {
		...
		this.session.on(
			this.options.event,
			(...args: unknown[]) => this.collect(...(args as Parameters<Handler[E]>)),
		);
		this.timeout = setTimeout(
			() => this.stop('time'),
			this.options.idle ?? this.options.time,
		);
	}
	...
	on(
		event: 'collect',
		listener: (...args: Parameters<Handler[E]>) => unknown,
	): this;
	on(
		event: 'end',
		listener: (
			reason: string | null | undefined,
			collected: Set<Parameters<Handler[E]>[0]>,
		) => void,
	): this;
