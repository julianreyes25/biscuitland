...
export class ApplicationManager {
	...
	getRoleConnectionMetadata(applicationId: string) {
		return this.session.api.applications(applicationId)['role-connections']
			.metadata.get();
	}
	...
	getCommand(applicationId: string, commandId: string) {
		return this.session.api.applications(applicationId).commands(commandId)
			.get();
	}
	...
	getCommandPermissions(
		...
		return this.session.api.applications(applicationId).guilds(guildId)
			.commands(commandId).permissions.get();
	}
	...
	editCommandPermissions(
		...
		return this.session.api.applications(applicationId).guilds(guildId)
			.commands(commandId).permissions.put({ body });
	}
	...
	getGuildCommands(
		...
		return this.session.api.applications(applicationId).guilds(guildId).commands
			.get({ query });
	}
