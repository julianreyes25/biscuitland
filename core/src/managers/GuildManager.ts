...
export class GuildManager {
	...
	createChannel<T extends APIGuildChannel<GuildChannelType>>(
		guildId: string,
		body: RESTPostAPIGuildChannelJSONBody,
	) {
		return this.session.api.guilds(guildId).channels.post({ body }) as Promise<
			T
		>;
	}
	...
	editRolePositions(
		guildId: string,
		options: RESTPatchAPIGuildRolePositionsJSONBody,
	) {
	...
	editRole(
		...
		return this.session.api.guilds(guildId).roles(roleId).patch({
			body: options,
		});
	}
	...
	deleteIntegration(guildId: string, integrationId: string) {
		return this.session.api.guilds(guildId).integrations(integrationId)
			.delete();
	}
	...
	editWidget(
		guildId: string,
		options: RESTPatchAPIGuildWidgetSettingsJSONBody,
	) {
	...
	editWelcomeScreen(
		guildId: string,
		options: RESTPatchAPIGuildWelcomeScreenJSONBody,
	) {
		return this.session.api.guilds(guildId)['welcome-screen'].patch({
			body: options,
		});
	}
	...
	getAutoModerationRule(guildId: string, ruleId: string) {
		return this.session.api.guilds(guildId)['auto-moderation'].rules(ruleId)
			.get();
	}
	...
	createAutoModerationRule(
		...
		return this.session.api.guilds(guildId)['auto-moderation'].rules.post({
			body,
			reason,
		});
	}
	...
	removeRoleMember(guildId: string, memberId: string, roleId: string) {
		return this.session.api.guilds(guildId).members(memberId).roles(roleId)
			.delete({});
	}
	...
	editCurrentVoiceState(
		...
		return this.session.api.guilds(guildId)['voice-states']['@me'].patch({
			body,
		});
	}

	editMemberVoiceState(
		guildId: string,
		memberId: string,
		body: RESTPatchAPIGuildVoiceStateUserJSONBody,
	) {
		return this.session.api.guilds(guildId)['voice-states'](memberId).patch({
			body,
		});
	}
	...
	getScheduledEvent(
		...
		return this.session.api.guilds(guildId)['scheduled-events'](scheduledEvent)
			.get({ query });
	}
