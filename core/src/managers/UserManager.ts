...
import type { ImageOptions, Session } from '../../mod.ts';
import { formatImageURL } from '../../mod.ts';
...
export class UserManager {
	...
	avatarURL(user: APIUser, { size, format }: ImageOptions = {}) {
		if (user.avatar?.length) {
			return formatImageURL(
				this.session.cdn.avatars(user.id).get(user.avatar),
				size,
				format,
			);
		}
		...
		return formatImageURL(
			this.session.cdn.embed.avatars.get(Number(user.discriminator) % 5),
		);
	}

	createDM(userId: string) {
		return this.session.api.users('@me').channels.post({
	...
	getRoleConnections(applicationId: string) {
		return this.session.api.users('@me').applications(
			applicationId,
		)['role-connection'].get();
	}
