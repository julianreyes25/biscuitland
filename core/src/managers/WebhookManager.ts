...
export class WebhookManager {
	...
	get(webhookdId: string, token?: string) {
		...
		return this.session.api.webhooks(webhookdId)(token as string).get();
	}
	...
	edit(webhookId: string, body: RESTPatchAPIWebhookJSONBody, token?: string) {
		...
		return this.session.api.webhooks(webhookId)(token as string).patch({
			body,
		});
	}
	...
	delete(webhookdId: string, token?: string) {
		...
		return this.session.api.webhooks(webhookdId)(token as string).delete();
	}
...
export type RESTPostAPIWebhookWithTokenWaitQuery = Identify<
	RESTPostAPIWebhookWithTokenQuery & { wait: true }
>;
