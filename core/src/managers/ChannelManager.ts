...
import type { RawFile } from '../../../rest/mod.ts';
...
export class ChannelManager {
	...
	getArchivedThreads(
		...
		if (type === 'private') {
			return this.session.api.channels(channelId).threads.archived.private.get({
				query,
			});
		}

		return this.session.api.channels(channelId).threads.archived.public.get({
			query,
		});
	}

	crosspostMessage(channelId: string, messageId: string) {
		return this.session.api.channels(channelId).messages(messageId).crosspost
			.post({});
	}

	createReaction(channelId: string, messageId: string, emoji: string) {
		return this.session.api.channels(channelId).messages(messageId).reactions(
			emoji,
		)('@me').put({});
	}
	...
	deleteMessage(channelId: string, messageId: string, reason?: string) {
		return this.session.api.channels(channelId).messages(messageId).delete({
			reason,
		});
	}
	...
	bulkMessages(
		...
		return this.session.api.channels(channelId).messages['bulk-delete'].post({
			body,
			reason,
		});
	}
	...
	editPermissions(
		...
		return this.session.api.channels(channelId).permissions(overwriteId).put({
			body,
			reason,
		});
	}
	...
	deletePermission(channelId: string, overwriteId: string, reason?: string) {
		return this.session.api.channels(channelId).permissions(overwriteId).delete(
			{ reason },
		);
	}
	...
	unpinMessage(channelId: string, messageId: string, reason?: string) {
		return this.session.api.channels(channelId).pins(messageId).delete({
			reason,
		});
	}
	...
	groupDMAddRecipient(
		...
		return this.session.api.channels(channelId).recipients(userId).put({
			body,
		});
	}
	...
	startThreadFromMessage(
		...
		return this.session.api.channels(channelId).messages(messageId).threads
			.post({ body, reason });
	}
	...
	getListJoinedPrivateArchivedThreads(
		channelId: string,
		query?: RESTGetAPIChannelThreadsArchivedQuery,
	) {
		return this.session.api.channels(channelId).users('@me').threads.archived
			.private.get({ query });
	}
	...
	getThreadMembers(
		...
		return this.session.api.channels(channelId)['thread-members'].get({
			query,
		});
	}
	...
	getThreadMember(
		...
		return this.session.api.channels(channelId)['thread-members'](userId).get({
			query,
		});
	}
	...
	removeThreadMember(channelId: string, userId: string) {
		return this.session.api.channels(channelId)['thread-members'](userId)
			.delete();
	}

	leaveThread(channelId: string) {
		return this.session.api.channels(channelId)['thread-members']('@me')
			.delete();
	}
	...
	editStageInstance(
		...
		return this.session.api['stage-instances'](channelId).patch({
			body,
			reason,
		});
	}
