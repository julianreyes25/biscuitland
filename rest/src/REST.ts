...
import type { Identify } from '../../common/mod.ts';
import type {
	RawFile,
	RequestData,
} from 'https://esm.sh/@discordjs/rest@2.2.0';
import { REST } from 'https://esm.sh/@discordjs/rest@2.2.0';
import type { RequestMethod } from './Router.ts';
...
export class BiscuitREST {
	...
	async get<T>(
		route: string,
		options?: RequestObject<RequestMethod.Get>,
	): Promise<T> {
	...
	async post<T>(
		route: string,
		body?: RequestObject<RequestMethod.Post>,
	): Promise<T> {
	...
	async put<T>(
		route: string,
		body?: RequestObject<RequestMethod.Put>,
	): Promise<T> {
	...
	async patch<T>(
		route: string,
		body?: RequestObject<RequestMethod.Patch>,
	): Promise<T> {
	...
	async delete<T>(
		route: string,
		options?: RequestObject<RequestMethod.Delete>,
	): Promise<T> {
...
> =
	...
	& RequestOptions
	& (M extends `${RequestMethod.Get}` ? unknown
		: {
			body?: B;
			files?: RawFile[];
		});
...
> = M extends RequestMethod.Get ? Q extends never ? RequestObject<M, never, B>
	: never
	: RequestObject<M, B, Q>;
